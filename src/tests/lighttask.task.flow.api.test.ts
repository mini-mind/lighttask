...
test("Task API：dispatch/start/request_approval/approve/complete 按终局状态机推进", () => {
  ...
  const running = lighttask.advanceTask(task.id, {
    action: "start",
  ...
  const waitingApproval = lighttask.advanceTask(task.id, {
    action: "request_approval",
    expectedRevision: running.revision,
  });
  assert.equal(waitingApproval.status, "blocked_by_approval");

  const resumed = lighttask.advanceTask(task.id, {
    action: "approve",
