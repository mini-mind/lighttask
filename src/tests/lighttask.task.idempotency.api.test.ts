...
test("LightTask 公共 API 在同幂等键重复推进时会拒绝语义不一致请求", () => {
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assert.equal(error.coreError.message, "相同 idempotencyKey 对应的请求内容不一致，拒绝处理。");
      assert.equal(error.details?.idempotencyKey, "req_1");
      assert.equal(typeof error.details?.incomingFingerprint, "string");
      assert.equal(typeof error.details?.storedFingerprint, "string");
      return true;
    },
  );
});
