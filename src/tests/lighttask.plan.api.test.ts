...
test("LightTask Plan API 支持列出计划且返回快照与内部状态隔离", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  ...
  const listed = lighttask.listPlans();
  assert.equal(listed.length, 2);
  assert.equal(listed[0].id, "plan_list_beta");
  assert.equal(listed[1].id, "plan_list_alpha");

  listed[1].title = "外部篡改";
  assert.ok(listed[1].metadata);
  listed[1].metadata.owner = { name: "mutated" };

  const stored = lighttask.getPlan("plan_list_alpha");
  assert.ok(stored);
  assert.equal(stored.title, "计划 Alpha");
  assert.deepEqual(stored.metadata, { owner: { name: "tester" } });
});

test("LightTask Plan API 支持基于 revision 更新计划结构化字段", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_success",
    title: "旧标题",
    metadata: { owner: { name: "tester" } },
    extensions: {
      properties: { priority: "p1" },
      namespaces: { planner: { lane: "draft" } },
    },
  });

  const updated = lighttask.updatePlan("  plan_update_success  ", {
    expectedRevision: 1,
    title: "  新标题  ",
    metadata: { owner: { name: "reviewer" } },
    extensions: {
      properties: { priority: "p2" },
      namespaces: { planner: { lane: "confirmed" } },
    },
  });

  assert.equal(updated.id, "plan_update_success");
  assert.equal(updated.title, "新标题");
  assert.equal(updated.status, "draft");
  assert.equal(updated.revision, 2);
  assert.deepEqual(updated.metadata, { owner: { name: "reviewer" } });
  assert.deepEqual(updated.extensions, {
    properties: { priority: "p2" },
    namespaces: { planner: { lane: "confirmed" } },
  });

  const stored = lighttask.getPlan("plan_update_success");
  assert.ok(stored);
  assert.equal(stored.title, "新标题");
  assert.equal(stored.revision, 2);
});

test("LightTask Plan API updatePlan 支持显式清空 metadata/extensions", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_clear",
    title: "清空字段",
    metadata: { owner: { name: "tester" } },
    extensions: {
      properties: { priority: "p1" },
  ...
  const updated = lighttask.updatePlan("plan_update_clear", {
    expectedRevision: 1,
    metadata: null,
    extensions: null,
  });
...
test("LightTask Plan API 推进返回快照应与内部状态隔离", () => {
  ...
  assert.ok(advanced.extensions);
  advanced.extensions.properties = { priority: "p2" };
...
test("LightTask Plan API updatePlan 空白 planId 时会抛校验错误", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());

  assert.throws(
    () =>
      lighttask.updatePlan("   ", {
        expectedRevision: 1,
        title: "不会生效",
      }),
    (error) => {
...
test("LightTask Plan API updatePlan 缺失 expectedRevision 时会抛校验错误", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_missing_revision",
    title: "缺失 revision",
  });

  assert.throws(
    () =>
      lighttask.updatePlan("plan_update_missing_revision", {
        title: "更新标题",
      } as never),
    (error) => {
      assert.ok(error instanceof LightTaskError);
      assert.equal(error.code, "VALIDATION_ERROR");
      assert.equal(error.coreError.message, "expectedRevision 为必填字段");
      assert.equal(error.details?.planId, "plan_update_missing_revision");
      return true;
    },
  );
});

test("LightTask Plan API updatePlan 未提供可更新字段时会抛校验错误", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_no_fields",
    title: "无字段更新",
  });

  assert.throws(
    () =>
      lighttask.updatePlan("plan_update_no_fields", {
        expectedRevision: 1,
      }),
    (error) => {
      assert.ok(error instanceof LightTaskError);
      assert.equal(error.code, "VALIDATION_ERROR");
      assert.equal(error.coreError.message, "更新计划时至少提供一个可变更字段");
      assert.equal(error.details?.planId, "plan_update_no_fields");
      return true;
    },
  );
});

test("LightTask Plan API updatePlan 空白标题会抛校验错误", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_blank_title",
    title: "原标题",
  });

  assert.throws(
    () =>
      lighttask.updatePlan("plan_update_blank_title", {
        expectedRevision: 1,
        title: "   ",
      }),
    (error) => {
...
test("LightTask Plan API updatePlan 在计划不存在时返回 NOT_FOUND", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());

  assert.throws(
    () =>
      lighttask.updatePlan("plan_update_missing", {
        expectedRevision: 1,
        title: "更新标题",
      }),
    (error) => {
      ...
      assert.equal(error.code, "NOT_FOUND");
      assert.equal(error.coreError.message, "未找到计划");
      assert.equal(error.details?.planId, "plan_update_missing");
      return true;
    },
  );
});

test("LightTask Plan API updatePlan expectedRevision 不匹配时返回 REVISION_CONFLICT", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_update_revision_conflict",
    title: "revision 冲突",
  });

  assert.throws(
    () =>
      lighttask.updatePlan("plan_update_revision_conflict", {
        expectedRevision: 2,
        title: "新标题",
      }),
    (error) => {
...
test("LightTask Plan API 只要求当前已落地 plan 依赖，不前置耦合 task/graph 能力", () => {
  ...
  const lighttask = createLightTask({
    ...
    planRepository: {
      list() {
        return storedPlan ? [structuredClone(storedPlan)] : [];
      },
      get(planId: string) {
  ...
  const listed = lighttask.listPlans();
  const updated = lighttask.updatePlan("plan_minimal_repo", {
    expectedRevision: 1,
    title: "最小计划仓储已更新",
  });
  ...
  assert.equal(lighttask.getPlan("plan_minimal_repo")?.title, "最小计划仓储已更新");
  assert.equal(updated.title, "最小计划仓储已更新");
  assert.equal(
    lighttask.advancePlan("plan_minimal_repo", { expectedRevision: 2 }).status,
    "planning",
  );
});
