...
import type { CreateLightTaskOptions, PersistedLightPlan } from "../core/types";
  ...
  const invalidOptionsCases = [
    {
      ...
      options: {
        planRepository: {
          create() {
            return { ok: true as const, plan: {} as PersistedLightPlan };
          },
        } as unknown as CreateLightTaskOptions["planRepository"],
      },
    },
    {
...
test("LightTask Plan API 只要求当前已落地 plan 能力", () => {
  let storedPlan: PersistedLightPlan | undefined;
  const lighttask = createLightTask({
    ...createTestLightTaskOptions(),
    planRepository: {
      get(planId: string) {
      ...
      create(plan: PersistedLightPlan) {
