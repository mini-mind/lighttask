...
test("LightTask Graph API 查询已发布图时会标准化 planId", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "任务一", "plan_graph_published_trim");
  lighttask.createPlan({
...
test("LightTask Graph API 支持在草稿图上做增量编辑，且再次发布前不影响已发布图", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const taskOne = createGraphTaskRef(lighttask, "节点一 v1", "plan_graph_edit_boundary");
  const taskTwo = createGraphTaskRef(lighttask, "节点二 v1", "plan_graph_edit_boundary");
  const taskOneV2 = createGraphTaskRef(lighttask, "节点一 v2", "plan_graph_edit_boundary");
  const taskThree = createGraphTaskRef(lighttask, "节点三", "plan_graph_edit_boundary");
  lighttask.createPlan({
  ...
  lighttask.saveGraph("plan_graph_edit_boundary", {
    nodes: [
      { id: "node_1", taskId: taskOne.id, label: "节点一 v1" },
      { id: "node_2", taskId: taskTwo.id, label: "节点二 v1" },
    ],
    edges: [{ id: "edge_1", fromNodeId: "node_2", toNodeId: "node_1", kind: "depends_on" }],
  });
  ...
  const edited = lighttask.editGraph("plan_graph_edit_boundary", {
    ...
    operations: [
      ...
      {
        type: "upsert_node",
        node: { id: "node_1", taskId: taskOneV2.id, label: "节点一 v2" },
      },
      {
        type: "upsert_node",
        node: { id: "node_3", taskId: taskThree.id, label: "节点三" },
      },
      {
        type: "upsert_edge",
  ...
  assert.deepEqual(
    ...
    [
      { id: "node_1", label: "节点一 v2", taskId: taskOneV2.id },
      { id: "node_3", label: "节点三", taskId: taskThree.id },
    ],
  );
...
test("LightTask Graph API 增量编辑后会拒绝把同一 taskId 映射到多个节点", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_graph_edit_duplicate_task_ref",
    title: "图编辑重复任务引用",
  });
  lighttask.saveGraph("plan_graph_edit_duplicate_task_ref", {
    nodes: [{ id: "node_1", taskId: "task_1", label: "节点一" }],
    edges: [],
  });
  ...
  expectLightTaskError(
    () =>
      lighttask.editGraph("plan_graph_edit_duplicate_task_ref", {
        expectedRevision: 1,
        operations: [
          {
            type: "upsert_node",
            node: { id: "node_2", taskId: "task_1", label: "节点一镜像" },
          },
        ],
      }),
    {
...
test("LightTask Graph API 支持发布草稿图并读取已发布图", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "草稿任务", "plan_graph_publish");
  lighttask.createPlan({
  ...
  lighttask.saveGraph("plan_graph_publish", {
    nodes: [{ id: "node_1", taskId: task.id, label: "草稿任务" }],
    edges: [],
  });
  ...
  assert.equal(lighttask.getPlan("plan_graph_publish")?.revision, 2);
});

test("LightTask Graph API 发布后草稿与已发布语义保持隔离，直到再次发布才刷新", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "草稿任务", "plan_graph_publish_boundary");
  lighttask.createPlan({
  ...
  lighttask.saveGraph("plan_graph_publish_boundary", {
    nodes: [{ id: "node_1", taskId: task.id, label: "草稿 v1" }],
    edges: [],
  });
  ...
  lighttask.saveGraph("plan_graph_publish_boundary", {
    expectedRevision: 1,
    nodes: [{ id: "node_1", taskId: task.id, label: "草稿 v2" }],
    edges: [],
  });
...
test("LightTask Graph API 发布图时会标准化 planId", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "任务一", "plan_graph_publish_trim");
  lighttask.createPlan({
...
test("LightTask Graph API 发布图时允许引用已归属当前计划的任务", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "待绑定任务", "plan_graph_claim_task");
  lighttask.createPlan({
    id: "plan_graph_claim_task",
    title: "发布绑定任务",
  });
  lighttask.saveGraph("plan_graph_claim_task", {
    nodes: [{ id: "node_claim_task", taskId: task.id, label: "待绑定任务" }],
    edges: [],
  });

  lighttask.publishGraph("plan_graph_claim_task", {
    expectedRevision: 1,
  });

  assert.equal(lighttask.getTask(task.id)?.planId, "plan_graph_claim_task");
});
...
test("LightTask Graph API 发布图时若节点引用不存在任务则返回 NOT_FOUND", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_publish_missing_task_ref",
    title: "发布缺失任务",
  });
  lighttask.saveGraph("plan_publish_missing_task_ref", {
  ...
  expectLightTaskError(
    () =>
      lighttask.publishGraph("plan_publish_missing_task_ref", {
        expectedRevision: 1,
      }),
    {
      code: "NOT_FOUND",
      message: "图节点引用的任务不存在，无法发布图快照",
      details: {
        planId: "plan_publish_missing_task_ref",
...
test("LightTask Graph API 发布图时若任务未归属当前计划则返回 STATE_CONFLICT", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const sharedTask = createGraphTaskRef(lighttask, "共享任务", "plan_publish_owner_a");
  lighttask.createPlan({
    id: "plan_publish_owner_a",
    title: "归属计划 A",
  });
  lighttask.saveGraph("plan_publish_owner_a", {
    nodes: [{ id: "node_owner_a", taskId: sharedTask.id, label: "共享任务" }],
    edges: [],
  });
  lighttask.publishGraph("plan_publish_owner_a", {
    expectedRevision: 1,
  });

  lighttask.createPlan({
    id: "plan_publish_owner_b",
    title: "归属计划 B",
  });
  lighttask.saveGraph("plan_publish_owner_b", {
    nodes: [{ id: "node_owner_b", taskId: sharedTask.id, label: "共享任务" }],
    edges: [],
  });

  expectLightTaskError(
    () =>
      lighttask.publishGraph("plan_publish_owner_b", {
        expectedRevision: 1,
      }),
    {
      code: "STATE_CONFLICT",
      message: "图节点引用的任务未归属当前计划，无法发布图快照",
      details: {
        planId: "plan_publish_owner_b",
        nodeId: "node_owner_b",
        taskId: sharedTask.id,
        taskPlanId: "plan_publish_owner_a",
      },
    },
  );
});
...
test("LightTask Graph API 在 confirmed 计划上拒绝再次发布图", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  const task = createGraphTaskRef(lighttask, "已启动任务", "plan_publish_confirmed_blocked");
  lighttask.createPlan({
    id: "plan_publish_confirmed_blocked",
    title: "启动后冻结发布",
  });
  lighttask.advancePlan("plan_publish_confirmed_blocked", { expectedRevision: 1 });
  lighttask.advancePlan("plan_publish_confirmed_blocked", { expectedRevision: 2 });
  lighttask.saveGraph("plan_publish_confirmed_blocked", {
    nodes: [{ id: "node_1", taskId: task.id, label: "已启动任务" }],
    edges: [],
  });
  lighttask.publishGraph("plan_publish_confirmed_blocked", {
    expectedRevision: 1,
  });
  lighttask.launchPlan("plan_publish_confirmed_blocked", {
    expectedRevision: 4,
    expectedPublishedGraphRevision: 1,
  });
  lighttask.saveGraph("plan_publish_confirmed_blocked", {
    expectedRevision: 1,
    nodes: [{ id: "node_1", taskId: task.id, label: "已启动任务 v2" }],
    edges: [],
  });
  ...
  expectLightTaskError(
    ...
    {
      code: "STATE_CONFLICT",
      message: "当前计划状态不允许发布图快照",
      details: {
        planId: "plan_publish_confirmed_blocked",
...
test("LightTask Graph API 会拒绝同一关系视图内重复引用同一 taskId", () => {
  const lighttask = createLightTask(createTestLightTaskOptions());
  lighttask.createPlan({
    id: "plan_graph_duplicate_task_ref",
    title: "重复任务引用",
  });

  expectLightTaskError(
    () =>
      lighttask.saveGraph("plan_graph_duplicate_task_ref", {
        nodes: [
          { id: "node_1", taskId: "task_shared", label: "任务一" },
          { id: "node_2", taskId: "task_shared", label: "任务一镜像" },
        ],
        edges: [],
      }),
    {
