import { createTaskRecord } from "../models";
...
export function createTaskUseCase(
  ...
  // 任务创建必须继承所属 Plan 的策略，避免不同计划之间共享错误的调度语义。
  const taskPolicy = requireTaskPolicyForPlan(options, plan);
  const status = taskPolicy.initialStatus;
