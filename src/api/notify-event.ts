import { createDomainEvent } from "../models";
...
export function publishPlanDeletedEvent(
  publishEvent: PublishEvent,
  input: {
  ...
  const event = createDomainEvent({
    id: createEventId("plan.deleted", input.result.planId, input.revision),
    type: "plan.deleted",
    aggregate: "plan",
    aggregateId: input.result.planId,
    occurredAt: input.occurredAt,
    revision: input.revision,
    payload: { result: input.result },
  }) as PlanDeletedEvent;
...
export function publishRuntimeDeletedEvent(
  publishEvent: PublishEvent,
  input: {
    result: DeleteRuntimeResult;
    occurredAt: string;
    revision: number;
  },
): RuntimeDeletedEvent {
  const event = createDomainEvent({
    id: createEventId("runtime.deleted", input.result.runtimeId, input.revision),
    type: "runtime.deleted",
    aggregate: "runtime",
    aggregateId: input.result.runtimeId,
    occurredAt: input.occurredAt,
    revision: input.revision,
    payload: { result: input.result },
...
export function publishOutputDeletedEvent(
  publishEvent: PublishEvent,
  input: {
  ...
  const event = createDomainEvent({
    id: createEventId("output.deleted", input.result.outputId, input.revision),
    type: "output.deleted",
    aggregate: "output",
    aggregateId: input.result.outputId,
    occurredAt: input.occurredAt,
    revision: input.revision,
    payload: { result: input.result },
  }) as OutputDeletedEvent;
  publishEvent(event);
  return event;
}
