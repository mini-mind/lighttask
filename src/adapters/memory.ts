...
>(): PlanRepository<TPlan> {
  ...
  return {
    ...
    deleteIfRevisionMatches(planId, expectedRevision) {
      const deleted = repository.deleteIfRevisionMatches(planId, expectedRevision);
      if (!deleted.ok) {
        return deleted;
      }
      return {
...
>(): RuntimeRepository<TRuntime> {
  ...
  return {
    ...
    deleteIfRevisionMatches(runtimeId, expectedRevision) {
      const deleted = repository.deleteIfRevisionMatches(runtimeId, expectedRevision);
      if (!deleted.ok) {
        return deleted;
      }
      return {
        ok: true as const,
        runtime: deleted.record,
      };
    },
  };
}
...
>(): OutputRepository<TOutput> {
  ...
  return {
    ...
    deleteIfRevisionMatches(outputId, expectedRevision) {
      const deleted = repository.deleteIfRevisionMatches(outputId, expectedRevision);
      if (!deleted.ok) {
        return deleted;
      }
      return {
...
export function createMemoryAdapters<TTaskPolicies = never>(
  ...
  return {
    ...
    taskPolicies: overrides.taskPolicies,
