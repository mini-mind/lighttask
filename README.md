...
说明：当前文档描述的是 LightTask 现已落地的 Task-only 内核模型；若需要看重构拆解与阶段验收，可补充参考 [docs/plan.md](https://github.com/mini-mind/lighttask/blob/main/docs/plan.md)，但真正面向接入的现行说明以 README、架构文档和应用层接入指南为准。

LightTask 是面向上层业务应用的 TypeScript 编排内核。它的终局方向不是“流程图引擎”，而是：
...
LightTask 以后不再要求你先画一张 `Graph` 再开始工作。  
...
这个最小启动示例已经对应当前实现。如果你想继续看设计约束与接入心智，优先阅读架构文档和应用层接入指南；`docs/plan.md` 主要保留为本轮重构的施工与验收记录。
...
- [PRD](https://github.com/mini-mind/lighttask/blob/main/docs/lighttask-kernel-replacement-prd.md)
- [重构计划](https://github.com/mini-mind/lighttask/blob/main/docs/plan.md)
